...
calcPi:
.LFB18: 
    ; отличие от -O0 -- отсутствует стандартный пролог функции
    ; проталкиывем edi, esi, ebx в стек, так как они будут активно
    ; использоваться в функции. В конце функции будет 3 инструкции pop
	.cfi_startproc
	push	edi
	...
	je	.L6 ; je == jz
	; зануляем edi (это M), ebx (это i)
	; используется xor, так как он занимает меньше байт
	; в исполняемом файле
	xor	edi, edi
	xor	ebx, ebx
	jmp	.L5
...
.L5:
	...
	fmul	QWORD PTR .LC1 ; rand() * ((2.0/(double)RAND_MAX) = const)
	fld1
	fsub	st(1), st ; отнимаем 1 от rand() * const
	fxch	st(1) ; в st(0) --  значение x, в st(1) -- единица
	fstp	QWORD PTR [esp+16]
	; чтобы для вычисления y единицу не грузить с помощью fld1, 
	; кладем ее на вершину стека
	fstp	QWORD PTR [esp] 
	...
	fild	DWORD PTR [esp+36]
	fmul	QWORD PTR .LC1
	fld	QWORD PTR [esp] ; в данном случае смысл как у fld1
	fsub	st(1), st
	fld	QWORD PTR [esp+16]
	; в этой точке состояние стека x87 такое:
	; st(0) = x
	; st(1) = 1.0
	; st(2) = y
	fmul	st, st(0)
	fxch	st(2)
	fmul	st, st(0)
	faddp	st(2), st ; после faddp в st(2) лежит xx+yy
	fucompp
	fnstsw	ax
	sahf
