...
import ru.vlitomsk.raytr.geom.render.intersect.Octree;
...
import ru.vlitomsk.raytr.geom.transform.*;
...
public class SceneRenderer {
    ...
    public SceneRenderer(Scene scene, ViewportCameraSettings camSettings) {
        this.scene = scene;
    ...
    private Octree<SceneObject> tree = new Octree<>(-2, 8, -2, 8, -2, 8);
    ...
    class JustRenderer {
        ...
        private boolean anyIsection;
        private double minSqrNorm;
        private SceneObject minIsectObj;
        private int iscounter;
        private Vec traceRay(Vec raypt, Vec normalizedDir, int depth) {
            //Vec minIsect = null;
            anyIsection = false;
            minIsectObj = null;
            minSqrNorm = Double.MAX_VALUE;
            iscounter = 0;

            //for (SceneObject obj : sceneObjects) {
            tree.allRayIntersections(raypt.clone(), normalizedDir.clone(), obj -> {
                ...
                if (isect == null)
                    return;
                    //continue;
                if (depth == 1 && obj.getOpticParams().power < 2000.5) {
                    System.out.print("");
                }
            ...
            if (depth >= 2) {
                ...
                Vec vmid = surfNorm.clone().pcoef3((surfNorm.dot(3, Evec))).pcoef3(1);
                refl[depth].assign3(vmid).padd(vmid.clone().padd(Evec,-1));

//                refl[depth].assign3(surfNorm).pcoef3(-2*surfNorm.dot(3,Evec)).padd(Evec);

                //refl[depth].assign(surfNorm).pcoef3(-2.0 * surfNorm.dot(3, Evec)).padd(Evec);
                Vec intensR = traceRay(minIsect.padd(normalizedDir, -0.01).clone(), refl[depth], depth - 1);
                if (intensR != null)
    ...
    private int count = 0;
    protected void refillTree() {
        double x1=Double.MAX_VALUE,y1=Double.MAX_VALUE,z1=Double.MAX_VALUE;
        double x2=Double.MIN_VALUE,y2=Double.MIN_VALUE,z2=Double.MIN_VALUE;
        for (SceneObject sobj : sceneObjects) {
            x1 = min(x1,sobj.minX());
            y1 = min(y1, sobj.minY());
            z1 = min(z1, sobj.minZ());
        ...
        tree = new Octree<>(x1-0.1,x2+0.1,y1-0.1,y2+0.1,z1-0.1,z2+0.1);
        for (SceneObject sobj : sceneObjects) {
            tree.add(sobj, 4);
        }
    }
    ...
    public long renderRaytraced(BufferedImage img, int raytracingDepth, double gamma, int qualitu) {
        //return justRenderer.renderRaytraced(img, raytracingDepth);
        refillTree();
        long tm = justRenderer.prerenderRaytraced(raytracingDepth, 0, img.getWidth(), 0, img.getHeight(), gamma, qualitu);
